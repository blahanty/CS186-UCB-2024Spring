...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> splitPair = getChild(index).put(key, rid);
        if (!splitPair.isPresent()) {
            sync();
            return Optional.empty();
        }

        keys.add(index, splitPair.get().getFirst());
        children.add(index + 1, splitPair.get().getSecond());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> splitPair = getChild(index).bulkLoad(data, fillFactor);
        if (!splitPair.isPresent()) {
            sync();
            return Optional.empty();
        }
