...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    private void rollbackToLSN(long transNum, long LSN) {
        ...
        while (currentLSN > LSN) {
            ...
            if (currentRecord.isUndoable()) {
                LogRecord CLRRecord = currentRecord.undo(lastRecordLSN);
    ...
    public synchronized void checkpoint() {
        ...
        for (Map.Entry<Long, Long> entry : dirtyPageTable.entrySet()) {
            if (!EndCheckpointLogRecord.fitsInOneRecord(chkptDPT.size() + 1, chkptTxnTable.size())) {
                LogRecord DPTRecord = new EndCheckpointLogRecord(chkptDPT, chkptTxnTable);
                logManager.appendToLog(DPTRecord);
                chkptDPT.clear();
                chkptTxnTable.clear();
            }
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            if (!EndCheckpointLogRecord.fitsInOneRecord(chkptDPT.size(), chkptTxnTable.size() + 1)) {
                LogRecord txnRecord = new EndCheckpointLogRecord(chkptDPT, chkptTxnTable);
                logManager.appendToLog(txnRecord);
                chkptDPT.clear();
                chkptTxnTable.clear();
            }
    ...
    void restartAnalysis() {
        ...
        while (iter.hasNext()) {
            ...
            if (currRecord.getTransNum().isPresent()) {
                ...
                Transaction txn = transactionEntry.transaction;
            ...
            if (currRecord.getPageNum().isPresent()) {
                ...
                switch (currRecord.getType()) {
                    ...
                    case UNDO_UPDATE_PAGE:
                        dirtyPageTable.putIfAbsent(pageNum, currLSN);
                        break;
                    case FREE_PAGE:
            ...
            if (currRecord.getType() == LogType.END_CHECKPOINT) {
                ...
                for (Map.Entry<Long, Pair<Transaction.Status, Long>> entry : currRecord.getTransactionTable().entrySet()) {
                    ...
                    if (transactionEntry.transaction.getStatus() == Transaction.Status.RUNNING || chkptStatus == Transaction.Status.COMPLETE) {
                        transactionEntry.transaction.setStatus(chkptStatus);
                        if (transactionEntry.transaction.getStatus() == Transaction.Status.ABORTING)
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            switch (txn.getStatus()) {
                ...
                case RUNNING:
                    ...
                    transactionTable.get(transNum).lastLSN = logManager.appendToLog(abortRecord);
                    break;
                default:
                    break;
            }
            ...
            if (record.isRedoable()) {
                switch (record.getType()) {
                    ...
                    case FREE_PAGE:
                        ...
                        long recLSN = record.getLSN();
                        if (recLSN < dirtyPageTable.get(pageNum))
                            break;
    ...
    void restartUndo() {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            if (transactionEntry.transaction.getStatus() == Transaction.Status.RECOVERY_ABORTING)
                abortingTransactions.add(new Pair<>(transactionEntry.lastLSN, transactionEntry));
        }

        while (!abortingTransactions.isEmpty()) {
            Pair<Long, TransactionTableEntry> lastTransaction = abortingTransactions.poll();
            long lastLSN = lastTransaction.getFirst();
            TransactionTableEntry transactionEntry = lastTransaction.getSecond();
            ...
            if (lastLSN == 0) {
                ...
                abortingTransactions.add(new Pair<>(lastLSN, transactionEntry));
        }
    }
