...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i : eligibleIndices) {
            ...
            IndexScanOperator indexOp = new IndexScanOperator(transaction, table, p.column, p.operator, p.value);
        ...
        return addEligibleSelections(minOp, exceptIndex);
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Set<Set<String>> prevTableSets = prevMap.keySet();
        for (Set<String> prevTableSet : prevTableSets) {
            for (JoinPredicate p : joinPredicates) {
                ...
                if (prevTableSet.contains(leftTable) && !prevTableSet.contains(rightTable)) {
                    ...
                    joinOp = minCostJoinType(prevMap.get(prevTableSet), rightOp, leftColumn, rightColumn);
                } else if (!prevTableSet.contains(leftTable) && prevTableSet.contains(rightTable)) {
                    newTableSet.add(leftTable);
                    tableSet.add(leftTable);
                    QueryOperator leftOp = pass1Map.get(tableSet);
                    joinOp = minCostJoinType(prevMap.get(prevTableSet), leftOp, rightColumn, leftColumn);
                } else
                    continue;

                if (result.containsKey(newTableSet)) {
                    if (joinOp.estimateIOCost() < result.get(newTableSet).estimateIOCost())
    ...
    public Iterator<Record> execute() {
        ...
        for (String tableName : tableNames) {
            ...
            pass1Map.put(tableSet, minCostSingleAccess(tableName));
        }

        Map<Set<String>, QueryOperator> prevMap = pass1Map;
        int passNumber = tableNames.size();
