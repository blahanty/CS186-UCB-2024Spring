...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                  float fillFactor) {
        int fullNum = (int) Math.ceil(2 * metadata.getOrder() * fillFactor);
        Pair<DataBox, RecordId> nextRecord;
        while (keys.size() < fullNum && data.hasNext()) {
        ...
        rightKeys.add(nextRecord.getFirst());
        List<RecordId> rightRids = new ArrayList<>();
        rightRids.add(nextRecord.getSecond());
        LeafNode rightNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, rightSibling, treeContext);
        long pageNum = rightNode.getPage().getPageNum();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSibling = pageID != -1 ? Optional.of(pageID) : Optional.empty();
        int n = buf.getInt();
        for (int i = 0; i < n; ++i) {
