...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        PriorityQueue<Pair<Record, Integer>> recordPairs = new PriorityQueue<>(new RecordPairComparator());
        for (int i = 0; i < runs.size(); ++i) {
            Iterator<Record> iter = runs.get(i).iterator();
            iters.add(iter);
            if (iter.hasNext()) {
                recordPairs.add(new Pair<>(iter.next(), i));
            }
        }

        Run mergedRun = new Run(transaction, getSchema());
        while (!recordPairs.isEmpty()) {
            Pair<Record, Integer> recordPair = recordPairs.poll();
            mergedRun.add(recordPair.getFirst());
            int i = recordPair.getSecond();
            if (iters.get(i).hasNext())
                recordPairs.add(new Pair<>(iters.get(i).next(), i));
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i + bufferSize < runSize; i += bufferSize) {
            mergedRuns.add(mergeSortedRuns(runs.subList(i, i + bufferSize)));
            index = i + bufferSize;
        }

        mergedRuns.add(mergeSortedRuns(runs.subList(index, runSize)));
        return mergedRuns;
    }
